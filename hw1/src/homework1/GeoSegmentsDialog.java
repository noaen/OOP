...
public class GeoSegmentsDialog extends JDialog implements ActionListener {
	...
	public GeoSegmentsDialog(Frame owner, RouteFormatterGUI pnlParent) {
		...
		//initializing list of geosegments, panel parent and scroll pane for the segments list
		...
		// creating add/cancel buttons and adding them to subpane
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (segIdx != -1) { // if a segment was selected
			if (this._lastAddedSeg == null) { // this is the first segment selected
				parent.addSegment(ExampleGeoSegments.segments[segIdx]);
				this._lastAddedSeg = ExampleGeoSegments.segments[segIdx];
			}
			else if (ExampleGeoSegments.segments[segIdx].getP1().equals(_lastAddedSeg.getP2())) { //if this is not the first selected seg., need to check it's legal to add
				parent.addSegment(ExampleGeoSegments.segments[segIdx]);
				this._lastAddedSeg = ExampleGeoSegments.segments[segIdx];
			}
		}

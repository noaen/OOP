...
/**
 * A NodeCountingPath characterizes a path of WeightedNodes. The cost for a path
 * is the number of WeightedNodes; the cost of the WeightedNodes are ignored.
 * <p>
...
public class NodeCountingPath implements Path<WeightedNode, NodeCountingPath> {
    ...
    /**
     * A NodeCountingPath which, when extended with 'node' at the end, is equal
     * to this. May be null iff this path has only 1 element.
     */
    ...
    /**
     * Creates a NodeCountingPath from a WeightedNode and a NodeCountingPath.
     * 
     * @requires node != null
     * @effects Creates a new NodeCountingPath 'res' such that res.elements =
     *          path.elements + [node]
     */
    private NodeCountingPath(WeightedNode node, NodeCountingPath path) {
        ...
        if (path != null)
            ...
            // <Itay> Modified
            this.cost = 1 + node.getBackEdges() + path.cost;
        else
            // TODO: update the following line to support back edges
            // <Itay> Modified
            this.cost = 1 + node.getBackEdges();
    }
     ...
     * 
     * @requires node != null
     * @return a new NodeCountingPath p such that p.elements == this.elements +
     *         [n] && p.cost >= this.cost
     */
     ...
     * 
     * @return an Iterator over the elements in the path in order from start to
     *         end.
     */
     ...
     * 
     * @return a string representation of this in the form [NodeCountingPath:
     *         node1, node2, node3, ...].
     */
     ...
     * 
     * @return true iff o is a NodeCountingPath and o.elements is the same
     *         sequence as this.elements
     */
    ...
    public boolean equals(NodeCountingPath wnp) {
        return (wnp != null) && this.node.equals(wnp.node)
     ...
     * 
     * @return the value 0 if the cost of this path is equal to the cost of the
     *         given path; a value less than 0 if this.cost is less than p.cost;
     *         and a value greater than 0 if this.cost is greater than p.cost.
